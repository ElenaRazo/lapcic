...
using TribunalAdministrativo.Medios.Atributos.Items;
using TribunalAdministrativo.Usuarios.Atributos;
using TribunalAdministrativo.Usuarios.Descriptores.Atributos;
using TribunalAdministrativo.Usuarios.Metodos;
using TribunalAdministrativo.Libreria.BaseDatos;

namespace TribunalAdministrativo.Medios.Metodos
{
    public class Multimedia : Libreria.BaseClass.BaseObject
    {
        public string Dominio { get; set; }

        TribunalAdministrativo.Usuarios.Metodos.UsuarioSeguridad us = new TribunalAdministrativo.Usuarios.Metodos.UsuarioSeguridad("Data Source=25.3.253.103;Initial Catalog=Usuarios;Persist Security Info=True;User ID=sa;Password=Rush01");
        ...
        public Tuple<long, long> CrearDirectorioDirectorioReal(TribunalAdministrativo.Medios.Atributos.Items.Directorio Directorio, string BaseUri, string Nombre, int IdOficina, int IdUsuario)
        {
            Tuple<long, long> resultado = new Tuple<long, long>(0, 0);
            var directorio = Directorio;
            var newUri = BaseUri + directorio.Archivo.Ruta + "\\" + Nombre;
            var DirUri = directorio.Archivo.Ruta + "\\" + Nombre;
            resultado = CrearDirectorioDirectorio(directorio.Id, "", (int)TribunalAdministrativo.Medios.Atributos.Items.ArchivoTipo.DIRECTORIO, Nombre, false, IdOficina, IdUsuario, true, DirUri, "", 1, 1, true, true);
            if (!System.IO.Directory.Exists(newUri))
            {
                System.IO.Directory.CreateDirectory(newUri);
            }
